...
uniform vec3            uBackgroundTextureRes;
...
void main()
{
	...
	vec2 pBackground1 = (p + heatAmount * offset(p, phase)) * uBackgroundTextureRes.xy;
	vec2 pBackground2 = (p + heatAmount * offset(p, phase + 0.5849)) * uBackgroundTextureRes.xy;
    
	vec4 s1 = texture(uBackgroundTexture, pBackground2);
	vec4 s2 = texture(uBackgroundTexture, pBackground2);

...
function closeAllArticles(){
    ...
    for (i = 0; i < accordionTitleButtons.length; i++) {
        // close other panels
        accordionTitleButtons[i].classList.add("panelNoTransition");
...
function checkToSortArticleList(){
    if(numArticleLoaded === numArticleToLoad) {
        // if all articles are loaded, display them all
...
function parseArticleFile(articleContent){
    ...
    firstImageFancybox.setAttribute("data-fancybox", title.innerHTML);
    ...
    for (let i =1; i < images.length; ++i){
        ...
        fancybox.setAttribute("data-fancybox", title.innerHTML);
        fancybox.href = images[i].getAttribute("src");
...
function addArticlesToList(articleElement){
    document.getElementById("contentArticleList").appendChild(articleElement.cloneNode(true));
}

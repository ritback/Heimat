...
uniform sampler2DRect   uVideoStream;             // input channel. XX = 2D/Cube
uniform vec3            uVideoStreamResolution;   // channel resolution (in pixels)

// -----------------------------------------------------------------------------

in vec2 iFragCoord; // coordinate in pixel.
in vec2 iVideoStreamCoord;
...
void main()
{
    vec2 p = uVideoStreamResolution.xy * iFragCoord.xy / uResolution.xy;
    oColor = texture(uVideoStream, p);
    //oColor = texture(uVideoStream, iVideoStreamCoord);
}
*/
// -----------------------------------------------------------------------------
...
{
	float THRESHOLD = 0.1;
	float time_s = mod( uGlobalTime, 32.0 );
	...
	vec4 smpl = texture( uVideoStream, uVideoStreamResolution.xy * uv_nm.xy );
    ...
    // Color
	//oColor = vec4( yuv2rgb(smpl_yuv), smpl.a );
    // BlackAndWhite
	oColor = vec4( yuv2rgb(smpl_yuv).r, yuv2rgb(smpl_yuv).r, yuv2rgb(smpl_yuv).r, smpl.a );
}
